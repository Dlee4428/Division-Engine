...
const vec3 quadPatches[] = vec3[] (	vec3(-0.5, 0.0, 0.5),
									vec3(0.5, 0.0, 0.5),
									vec3(0.5, 0.0, -0.5),
									vec3(-0.5, 0.0, -0.5));
...
void main() {
	...
	vec3 instPos = (quadPatches[gl_VertexID] * vec3(patchSizeWorld.x, 1, patchSizeWorld.y)) + 
							vec3(terrainOffset.x, 0, terrainOffset.y);
